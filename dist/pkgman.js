...
export function getDefaultLocalData() {
    return path.join(import.meta.dirname, "data-files");
}
export function getDefaultLaunchFile() {
    return {
        win: "camoufox.exe",
        mac: "../MacOS/camoufox",
        lin: "camoufox-bin",
...
export const INSTALL_DIR = getDefaultInstallDir();
export const LOCAL_DATA = getDefaultLocalData();
export const LAUNCH_FILE = getDefaultLaunchFile();
class Version {
    release;
    version;
    sorted_rel;
    ...
    static fromPath(filePath = getDefaultInstallDir()) {
...
export class CamoufoxFetcher extends GitHubDownloader {
    ...
    async extractZip(zipFile, installDir = getDefaultInstallDir()) {
        const zip = new AdmZip(zipFile);
        zip.extractAllTo(installDir.toString(), true);
    }
    static cleanup(installDir = getDefaultInstallDir()) {
    ...
    setVersion(installDir = getDefaultInstallDir()) {
        fs.writeFileSync(path.join(installDir.toString(), "version.json"), JSON.stringify({ version: this.version, release: this.release }));
    }
    async install(paths) {
...
export function installedVerStr(paths) {
    const installDir = paths?.installDir || getDefaultInstallDir();
    return Version.fromPath(installDir).fullString;
}
export function camoufoxPath(downloadIfMissing = true, paths) {
    const installDir = paths?.installDir || getDefaultInstallDir();
...
export function getPath(file, paths) {
    const installDir = paths?.installDir || getDefaultInstallDir();
    const resolvedPath = camoufoxPath(true, paths);
    if (OS_NAME === "mac") {
        return path.resolve(resolvedPath.toString(), "Camoufox.app", "Contents", "Resources", file);
    }
    return path.join(resolvedPath.toString(), file);
}
export function launchPath(paths) {
    const launchFile = paths?.launchFile || getDefaultLaunchFile();
    const launchPath = getPath(launchFile[OS_NAME], paths);
    if (!fs.existsSync(launchPath)) {
        throw new CamoufoxNotInstalled(`Camoufox is not installed at ${camoufoxPath(true, paths)}. Please run \`camoufox fetch\` to install.`);
    }
    return launchPath;
}
